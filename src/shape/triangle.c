...
** Last update Wed May  4 02:49:55 2016 Antoine Baché
*/
				  ...
				  t_vec3 *edge)
{
  ...
  if ((det > -0.000001 && det < 0.000001) || det == 0.0)
    return ;
  inv_det = 1.0 / det;
  dist = sub_vec3((t_vec3){0.0, 0.0, 0.0}, obj->spec.poly[0]);
  u = dot_vec3(dist, det_tmp) * inv_det;
  tmp = cross_vec3(dist, edge[0]);
  v = dot_vec3(ray->dist, tmp) * inv_det;
...
t_intersect	get_intersect_triangle(t_obj *obj, t_ray *ray)
{
  ...
  inter->dist = -1.0;
  edge[0] = sub_vec3(obj->spec.poly[1], obj->spec.poly[0]);
  edge[1] = sub_vec3(obj->spec.poly[2], obj->spec.poly[0]);
