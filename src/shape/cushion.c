...
** Last update Sun May 22 20:42:12 2016 Antoine Baché
*/
...
static void	get_dist_cushion(t_ray *ray, t_intersect *inter, t_vec3 tmp)
{
  double	t[5];
...
t_intersect	get_intersect_cushion(t_obj *obj, t_ray *ray)
{
  ...
  inter.mat = obj->mat;
  inter.dist = -1.0;
  get_dist_cushion(ray, &inter, ray->pos);
  if (inter.dist <= 0.0)
    return (inter);
