...
** Last update Sun May 15 02:50:04 2016 Antoine Baché
*/
				       ...
				       t_intersect *inter, t_vec3 tmp)
{
  ...
  s[2] = 6.0 * tmp.x * tmp.x * ray->dir.x * ray->dir.x + 2.0 * tmp.y * tmp.y * ray->dir.x * ray->dir.x + 2.0 * tmp.z * tmp.z * ray->dir.x * ray->dir.x + 2.0 * obj->sphere.radius * obj->sphere.radius * ray->dir.x * ray->dir.x + 2.0 * tmp.x * tmp.x * ray->dir.y * ray->dir.y + 6.0 * tmp.y * tmp.y * ray->dir.y * ray->dir.y + 2.0 * tmp.z * tmp.z * ray->dir.y * ray->dir.y + 2.0 * obj->sphere.radius * obj->sphere.radius * ray->dir.y * ray->dir.y + 2.0 * tmp.x * tmp.x * ray->dir.z * ray->dir.z + 2.0 * tmp.y * tmp.y * ray->dir.z * ray->dir.z + 6.0 * tmp.z * tmp.z * ray->dir.z * ray->dir.z + 2.0 * obj->sphere.radius * obj->sphere.radius * ray->dir.z * ray->dir.z + 8.0 * tmp.x * tmp.y * ray->dir.x * ray->dir.y + 8.0 * tmp.x * tmp.z * ray->dir.x * ray->dir.z + 8.0 * tmp.y * tmp.z * ray->dir.y * ray->dir.z;
  s[3] = 4.0 * tmp.x * tmp.y * tmp.y * ray->dir.x + 4.0 * tmp.x * tmp.z * tmp.z * ray->dir.x + 4.0 * tmp.x * tmp.x * tmp.x * ray->dir.x + 4.0 * obj->sphere.radius * obj->sphere.radius * tmp.x * ray->dir.x + 4.0 * tmp.y * tmp.z * tmp.z * ray->dir.y + 4.0 * tmp.y * tmp.y * tmp.y * ray->dir.y + 4.0 * tmp.x * tmp.x * tmp.y * ray->dir.y + 4.0 * obj->sphere.radius * obj->sphere.radius * tmp.y * ray->dir.y + 4.0 * tmp.z * tmp.z * tmp.z * ray->dir.z + 4.0 * tmp.x * tmp.x * tmp.z * ray->dir.z + 4.0 * tmp.y * tmp.y * tmp.z * ray->dir.z + 4.0 * obj->sphere.radius * obj->sphere.radius * tmp.z * ray->dir.z;
  s[4] = 2.0 * obj->sphere.radius * obj->sphere.radius * tmp.x * tmp.x + 2.0 * tmp.x * tmp.x * tmp.y * tmp.y + 2.0 * obj->sphere.radius * obj->sphere.radius * tmp.y * tmp.y + 2.0 * tmp.x * tmp.x * tmp.z * tmp.z + 2.0 * tmp.y * tmp.y * tmp.z * tmp.z + 2.0 * obj->sphere.radius * obj->sphere.radius * tmp.z * tmp.z + tmp.x * tmp.x * tmp.x * tmp.x + tmp.y * tmp.y * tmp.y * tmp.y + tmp.z * tmp.z * tmp.z * tmp.z + obj->sphere.radius * obj->sphere.radius * obj->sphere.radius * obj->sphere.radius;
...
t_intersect	get_intersect_double_sphere(t_obj *obj, t_ray *ray)
{
  ...
  inter.mat = obj->mat;
  inter.dist = -1.0;
  get_dist_double_sphere(obj, ray, &inter, sub_vec3(ray->pos, obj->pos));
  if (inter.dist <= 0.0)
    return (inter);
  return (inter);
}
